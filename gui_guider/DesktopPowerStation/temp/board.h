...
/* Default Dots Per Inch(px/inch). Used to initialize default sizes such as widgets sized, style paddings.
...
*LV_DRAW_SW_SHADOW_CACHE_SIZE is the maximum. shadow size to buffer, where shadow size is `shadow_width + radius`
...
#endif    /* LV_USE_DRAW_G2D && LV_USE_OS */
...
/* Enable assertion failures if an operation is failed or an invalid data is found.
...
/* Set an upper-case driver-identifier letter for this driver (e.g. 'A'). */
...
/*This needs to be adjusted according to size of the screen. */
...
#if LV_USE_WAYLAND
/* Use 1 for single buffer with partial render mode or 2 for double buffer with full render mode */
#define LV_WAYLAND_BUF_COUNT 1
/* Use DMA buffers for frame buffers. Requires LV_DRAW_USE_G2D */
#define LV_WAYLAND_USE_DMABUF 0
/* DMABUF supports LV_DISPLAY_RENDER_MODE_FULL and LV_DISPLAY_RENDER_MODE_DIRECT
